...
import { db, ideas, groups, groupMembers, domains, users, joinRequests, groupMessages, skills, ideaLookingFor, ideaImages, savedIdeas, eq, and, isNull, count, sql, lt, desc } from "../../../packages/db/index";
import { supabase, supabaseBucket } from "../services/supabaseClient";
...
export const uploadIdeaImage = async (req: AuthenticatedRequest, res: Response) => { const userId=req.user?.userId; const id=req.params.id as string; if(!userId)return res.status(401).json({error:"Unauthorized"}); if(!req.file)return res.status(400).json({error:"Image file is required"}); try { const [idea]=await db.select().from(ideas).where(eq(ideas.id,id)); if(!idea)return res.status(404).json({error:"Idea not found"}); const [group]=await db.select().from(groups).where(eq(groups.ideaId,id)); if(idea.ownerId!==userId && (!group || !await checkContributorAccess(group.id,userId)))return res.status(403).json({error:"Active contributor access required"}); const ext=req.file.mimetype.split("/")[1]||"bin"; const path=`ideas/${id}/${randomUUID()}.${ext}`; const {error}=await supabase.storage.from(supabaseBucket).upload(path,req.file.buffer,{contentType:req.file.mimetype}); if(error)throw error; const {data}=supabase.storage.from(supabaseBucket).getPublicUrl(path); const [image]=await db.insert(ideaImages).values({ideaId:id,userId,imageUrl:data.publicUrl}).returning(); return res.status(201).json({id:image.id,imageUrl:image.imageUrl}); }catch(error){console.error("Upload image error:",error);return res.status(500).json({error:"Image upload failed"});} };
export const deleteIdeaImage = async (req: AuthenticatedRequest,res:Response)=>{const userId=req.user?.userId;if(!userId)return res.status(401).json({error:"Unauthorized"});try{const [image]=await db.select().from(ideaImages).where(and(eq(ideaImages.id,req.params.imageId as string),eq(ideaImages.ideaId,req.params.id as string)));if(!image)return res.status(404).json({error:"Image not found"});const [idea]=await db.select().from(ideas).where(eq(ideas.id,image.ideaId));if(!idea|| (idea.ownerId!==userId&&image.userId!==userId))return res.status(403).json({error:"Only owner or uploader can delete image"});const marker=`/storage/v1/object/public/${supabaseBucket}/`;const path=image.imageUrl.split(marker)[1];if(path){const {error}=await supabase.storage.from(supabaseBucket).remove([path]);if(error)throw error;}await db.delete(ideaImages).where(eq(ideaImages.id,image.id));return res.json({message:"Image deleted"});}catch(error){console.error("Delete image error:",error);return res.status(500).json({error:"Image deletion failed"});}};
export const forkIdea=async(req:AuthenticatedRequest,res:Response)=>{const userId=req.user?.userId;if(!userId)return res.status(401).json({error:"Unauthorized"});try{const [source]=await db.select().from(ideas).where(eq(ideas.id,req.params.id as string));if(!source)return res.status(404).json({error:"Idea not found"});const [idea]=await db.insert(ideas).values({ownerId:userId,title:source.title,description:source.description,domainId:source.domainId,communityId:source.communityId,accessType:"open_open",forkedFromId:source.id}).returning();const [group]=await db.insert(groups).values({ideaId:idea.id}).returning();await db.insert(groupMembers).values({groupId:group.id,userId});return res.status(201).json({idea,group});}catch(error){console.error("Fork idea error:",error);return res.status(500).json({error:"Internal server error"});}};
export const getForkCount=async(req:AuthenticatedRequest,res:Response)=>{const [result]=await db.select({count:count()}).from(ideas).where(eq(ideas.forkedFromId,req.params.id as string));return res.json({count:Number(result.count)});};
export const saveIdea=async(req:AuthenticatedRequest,res:Response)=>{const userId=req.user?.userId;if(!userId)return res.status(401).json({error:"Unauthorized"});const id=req.params.id as string;const [idea]=await db.select().from(ideas).where(eq(ideas.id,id));if(!idea)return res.status(404).json({error:"Idea not found"});const [existing]=await db.select().from(savedIdeas).where(and(eq(savedIdeas.userId,userId),eq(savedIdeas.ideaId,id)));if(!existing)await db.insert(savedIdeas).values({userId,ideaId:id});return res.json({message:"Idea saved"});};
export const unsaveIdea=async(req:AuthenticatedRequest,res:Response)=>{const userId=req.user?.userId;if(!userId)return res.status(401).json({error:"Unauthorized"});await db.delete(savedIdeas).where(and(eq(savedIdeas.userId,userId),eq(savedIdeas.ideaId,req.params.id as string)));return res.json({message:"Idea unsaved"});};
